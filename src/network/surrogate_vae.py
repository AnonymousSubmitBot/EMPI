...
class SurrogateVAE(nn.Module):
    ...
    def forward_only_score(self, input: Tensor, **kwargs) -> Tensor:
        mu, log_var = self.encode(input)
        z = self.reparameterize(mu, log_var)
        return self.performance_predictor(z)
    ...
    def loss_function(self, X: Tensor, y: Tensor) -> dict:
        ...
        recons_result = (recons > 0.5).float()
        recon_acc = mean_absolute_error(recons_result.cpu().detach().numpy(), X.cpu().detach().numpy())
    ...
    def mapping_loss_function(self, X: Tensor, y: Tensor) -> dict:
        ...
        recons_result = (recons > 0.5).float()
        recon_acc = mean_absolute_error(recons_result.cpu().detach().numpy(), y.cpu().detach().numpy())

        return {'loss': loss, "Reconstruction_Loss": recons_loss.cpu().detach().numpy(), "Recon_Acc": recon_acc}
