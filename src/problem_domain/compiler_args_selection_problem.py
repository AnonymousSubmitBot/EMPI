...
class CompilerArgsSelectionProblem(BaseProblem):
    def __init__(self, dimension: int, train: bool = False, **kwargs):
        ...
        _candidate_program_index = _candidate_train_index + _candidate_eval_index
    ...
    def initial_ck(self, solution):
        ...
        compile_flag = self.get_opt(opt_list)
        program = self.program_dict['program']
        return -self.get_compile_size_old(program, compile_flag)


def main(initial=False):
    from pytorch_lightning import seed_everything
    seed_everything(1088, True)
    for id in _candidate_eval_index:
        if id == 15:
            continue
        solutions = [np.random.randint(2, size=100) for _ in range(10)]
        st = time.time()
        problem = CompilerArgsSelectionProblem(dimension=100, program_id=id)
        if initial:
            A1 = [problem.initial_ck(solution) for solution in solutions]
        else:
            A1 = [problem.evaluate(solution) for solution in solutions]
        print(id, problem.program_dict["program"], (time.time() - st),
              np.min(A1),
              )
