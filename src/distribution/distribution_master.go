...
func (dm *DistributedMaster) Run() {
	...
	router.POST("/get_multi_result", dm.getMultiResult)
...
func (dm *DistributedMaster) getMultiResult(c *gin.Context) {
	taskIDs := c.PostFormArray("task_id")
	...
	for _, taskID := range taskIDs {
		...
		f, err := zipWriter.Create(taskID)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"success": false, "msg": "Zip Results Failed"})
			return
		}
