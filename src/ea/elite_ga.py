...
class EliteGA(EA):
    ...
    def __init__(self, eval_func: Callable[[NpArray], float], dim: int = 30, pop_size: int = 32,
                 mut_prob: float = 0.001, max_iter: int = 100, max_eval: int = 800, elite=True, elite_k: int = None,
    ...
    def add_quality_record(self, solution: Union[NpArray, List], quality: float):
        solution_str = "".join([str(i) for i in solution])
        self.quality_table[solution_str] = quality
    ...
    def initial_population(self):
        ...
        if self.initial_solutions is not None:
            if len(self.initial_solutions) > self.pop_size:
                [self.get_quality(solution) for solution in self.initial_solutions[self.pop_size:]]
            for solution in self.initial_solutions[:self.pop_size]:
...
def test():
    ...
    ga = EliteGA(eval_func=one_max_fitness, dim=40, pop_size=64, initial_solutions=np.load(
